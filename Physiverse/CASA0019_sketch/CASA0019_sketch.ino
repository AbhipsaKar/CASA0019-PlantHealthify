  ...
  if (strstr(topic, "moisture") != NULL) //Sort the list of topics to find moisture topics
  {
    ...
    while (len != 0 && i < len ) {
      if ((s.indexOf(Array_topic[i])) > 0) {
        ...
        isElementPresent = 1;
        Serial.println("Element present at ");
        Serial.print(elementAtIndex);
        break;
      }
      i++;
    }
    ...
    if (!(isElementPresent))
    {
      ...
      //strncpy(Array_topic[len], topic + 23, ((strlen(topic)-23))*sizeof(char)); //Push only the last part of the topic name
      int j=0;
      int k =23;
      while(k <= strlen(topic))
      {
        Array_topic[len][j] = topic[k]; //Copy each character of the topic array
        j++;
        k++;
      }
      Array_topic[len][j] = '\0';
      strncpy(Array_value[len], messageTemp, (length + 1)*sizeof(char)); //Push array value into a separate array
      Serial.print(Array_topic[len]);
      Serial.print("\n");
      len ++;
    }
    else {
...
void loop() {
  ...
  if (now - lastMsg > 5000) { //Update LCD and gauge pointer at fixed intervals
    ...
    //Print the second line of LCD in sets of 16 characters
    for (int thisChar = 0; thisChar < topics.length(); thisChar++) {

      if (thisChar > 0 && thisChar % 16 == 0)
      { 
        delay(2000);
        loopMQTT(); //Check the MQTT connection after delay
        lcd.clear();
        lcd.setCursor(0, 0);
      ...
      lcd.print(topics[thisChar]);
     // Serial.println(topics[thisChar]);
